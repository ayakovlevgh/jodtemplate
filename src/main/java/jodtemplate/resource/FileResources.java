...
public class FileResources implements Resources {
    ...
    public FileResources(final File directory) {
        this.directory = directory;
    }

    @Override
    public Resource createResource(final String path) throws IOException {
        final File target = new File(directory, path);
        Utils.createParentFolders(target);
        final Resource resource = new FileResource(target, path);
